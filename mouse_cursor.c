...
static void drawButton(xcb_connection_t * connection, xcb_screen_t * screen, xcb_window_t window, 
...
static xcb_gc_t getFontGC(xcb_connection_t * connection, xcb_screen_t * screen, xcb_window_t window, 
							const char *fontName)
{
	...
	while (1) {
		...
		if (event) {
			...
			case XCB_EXPOSE:{
					...
					drawButton(connection, screen, window, (WIDTH - 7 * strlen(text)) / 2, (HEIGHT - 16) / 2, text);
